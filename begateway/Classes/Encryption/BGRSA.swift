...
typealias BGPadding = SecPadding
...
enum BGRSA {
    ...
    static func base64String(pemEncoded pemString: String) throws -> String {
        ...
        guard lines.count != 0 else {
            throw BGRSAError.pemDoesNotContainKey
        }
    ...
    static func data(forKeyReference reference: SecKey) throws -> Data {
        ...
        if #available(iOS 10.0, *), #available(watchOS 3.0, *), #available(tvOS 10.0, *) {
            ...
            guard let unwrappedData = data as Data? else {
                throw BGRSAError.keyRepresentationFailed(error: error?.takeRetainedValue())
            }
            ...
            guard let unwrappedData = data as? Data else {
                throw BGRSAError.keyAddFailed(status: addStatus)
            }
    ...
    static func generateRSAKeyPair(sizeInBits size: Int) throws -> (privateKey: BGPrivateKey, publicKey: BGPublicKey) {
        return try generateRSAKeyPair(sizeInBits: size, applyUnitTestWorkaround: false)
    }
    
    @available(iOS 10.0, watchOS 3.0, tvOS 10.0, *)
    static func generateRSAKeyPair(sizeInBits size: Int, applyUnitTestWorkaround: Bool = false) throws -> (privateKey: BGPrivateKey, publicKey: BGPublicKey) {
        ...
        guard let privKey = SecKeyCreateRandomKey(attributes as CFDictionary, &error),
            let pubKey = SecKeyCopyPublicKey(privKey) else {
                throw BGRSAError.keyGenerationFailed(error: error?.takeRetainedValue())
        }
        let privateKey = try BGPrivateKey(reference: privKey)
        let publicKey = try BGPublicKey(reference: pubKey)
    ...
    static func addKey(_ keyData: Data, isPublic: Bool, tag: String) throws ->  SecKey {
        ...
        if #available(iOS 10.0, *), #available(watchOS 3.0, *), #available(tvOS 10.0, *) {
            ...
            guard let key = SecKeyCreateWithData(keyData as CFData, keyDict as CFDictionary, &error) else {
                throw BGRSAError.keyCreateFailed(error: error?.takeRetainedValue())
            }
            return key
            ...
            guard let unwrappedKeyRef = keyRef else {
                throw BGRSAError.keyCopyFailed(status: copyStatus)
            }
    ...
    static func stripKeyHeader(keyData: Data) throws -> Data {
        ...
        let node: BGAsn1Parser.Node
        do {
            node = try BGAsn1Parser.parse(data: keyData)
        } catch {
            throw BGRSAError.asn1ParsingFailed
        }
        
        // Ensure the raw data is an ASN1 sequence
        guard case .sequence(let nodes) = node else {
            throw BGRSAError.invalidAsn1RootNode
        }
        ...
        throw BGRSAError.invalidAsn1Structure
    }
